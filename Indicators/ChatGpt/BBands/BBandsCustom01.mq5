   ...
   for(int i=pos; i<rates_total && !IsStopped(); i++)
     {
      ...
      int barShift = iBarShift(_Symbol,PERIOD_H1,time,false);


      double closeArray[20];
      CopyClose(_Symbol, PERIOD_H1, 0, InpBandsPeriod, closeArray);


      ExtMLBuffer[i]=SimpleMA(barShift,ExtBandsPeriod,closeArray);
      ExtStdDevBuffer[i]=StdDev_Func(barShift,closeArray,ExtMLBuffer,ExtBandsPeriod);
